...
sub limit_for {
    my (%args) = @_;
    my $kind = lc($args{kind} // '');
    # BOS/CHoCH: no cull por recencia. Si el engine los calculó y están en
    # viewport + toggle ON, deben dibujarse (1:1 con StructureEvent).
    return undef if $kind eq 'bos' || $kind eq 'choch';
...
sub resolve_collisions {
    ...
    for my $item (@ordered) {
        ...
        if ($item->{fixed_position}) {
            # BOS/CHoCH: fixed_position ancla el span al nivel; si hay colisión
            # de etiqueta NO ocultar el evento completo (línea + tag). Dibujar
            # aunque solape: preferimos 1:1 engine→gráfico sobre declutter.
            my $text = $item->{text} // '';
            if ($item->{protected} && $text =~ /^(?:BOS|CHoCH)/i) {
