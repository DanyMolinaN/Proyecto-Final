...
sub new {
    ...
    # Phase 2 Engines
    my $trailing_extremes_engine = $args{trailing_extremes_engine} || Market::Indicators::TrailingExtremes->new();
    my $premium_discount_engine = $args{premium_discount_engine} || Market::Concepts::PremiumDiscountZones->new();
    my $mtf_levels_engine = $args{mtf_levels_engine} || Market::Concepts::MTFLevels->new();
    ...
    my $self = {
        ...
        # Phase 2
        trailing_extremes_engine => $trailing_extremes_engine,
        premium_discount_engine  => $premium_discount_engine,
        mtf_levels_engine        => $mtf_levels_engine,
...
sub invalidate_analysis_cache {
    ...
    for my $key (qw(liquidity_engine fvg_engine orderblock_engine volume_profile_engine anchored_vwap fibonacci_engine supply_demand_engine trend_channel_engine trailing_extremes_engine premium_discount_engine mtf_levels_engine)) {
        my $eng = $self->{$key};
...
sub rebuild_analysis_cache {
    ...
    for my $key (qw(liquidity_engine fvg_engine orderblock_engine smc_structure_engine volume_profile_engine anchored_vwap fibonacci_engine supply_demand_engine trend_channel_engine trailing_extremes_engine premium_discount_engine mtf_levels_engine)) {
        my $eng = $self->{$key};
        $eng->reset() if $eng && $eng->can('reset');
    }
    ...
    # FibonacciEngine v2: recibe el HASH de SMC (no el objeto motor) — Fix Fase 0
    ...
    # TrendChannel Fix Fase 0: combinar swing_highs/lows (clave 'level') y
    # normalizar a 'price'+'type' que usa TrendChannel internamente.
    my @_tc_raw_swings = (
        @{ $smc_structure_data->{swing_highs} || [] },
        @{ $smc_structure_data->{swing_lows}  || [] },
    );
    my @_tc_combined_swings = map {
        my $sw = $_;
        my $lbl = $sw->{label} // '';
        {
            index => $sw->{index},
            price => $sw->{level},   # TrendChannel usa 'price', SMC usa 'level'
            # HH/LH son swing highs; HL/LL son swing lows
            type  => ($lbl eq 'HH' || $lbl eq 'LH') ? 'high' : 'low',
            label => $lbl,
        }
    } grep { ref $_ eq 'HASH' && defined $_->{index} && defined $_->{level} } @_tc_raw_swings;
    my $trend_channel_data = $self->{trend_channel_engine}->calculate(
        $self->{market_data}, source_swings => \@_tc_combined_swings, %engine_args,
    );
    
    # Phase 2 Calculations
    my $trailing_extremes_data = $self->{trailing_extremes_engine}->calculate(
        $self->{market_data}, $smc_structure_data, %engine_args,
    );
    my $premium_discount_data = $self->{premium_discount_engine}->calculate(
        $self->{market_data}, $trailing_extremes_data, %engine_args,
    );
    my $mtf_levels_data = $self->{mtf_levels_engine}->calculate(
        $self->{market_data}, %engine_args,
    );
