package Market::Indicators::AnchoredVWAPDavid;

# =============================================================================
# Market::Indicators::AnchoredVWAPDavid
#
# Portado desde Proyecto_David/Market/Indicators/AnchoredVWAP.pm
# Adaptaciones para Kevin (mismo patron que FibonacciDavid.pm):
#   - Package renombrado a AnchoredVWAPDavid (evita colision con
#     Market::Indicators::AnchoredVWAP legacy de Kevin, que tiene ancla fija).
#   - Se agrega recompute($md) para IndicatorManager::rebuild_all()/rebuild_one().
#   - Resto del algoritmo (pivote automatico + ancla manual + bandas de
#     desviacion ponderadas por volumen) es identico al original de David.
# =============================================================================
...
# recompute($md): contrato de IndicatorManager::rebuild_all()/rebuild_one() de
# Kevin. Reconstruye desde cero iterando todas las velas del MarketData activo.
sub recompute {
    my ( $self, $md ) = @_;
    return unless $md;
    $self->reset();
...
# { anchor_index, anchor_price, from_index, to_index, points }
# donde points es arrayref ordenado por indice con
...
# Precio tipico hlc3 = (high+low+close)/3. Volumen 0/undef -> peso minimo 1.
# -----------------------------------------------------------------------------
